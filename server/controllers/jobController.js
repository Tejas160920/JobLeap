...
exports.getJobs = async (req, res) => {
  try {
    ...
    // Sort: GitHub sources first (simplify, speedyapply), then by date
    const [jobs, totalJobs] = await Promise.all([
      Job.aggregate([
            ...
            sourcePriority: {
              $switch: {
                branches: [
                  ...
                  { case: { $eq: ['$source', 'speedyapply'] }, then: 2 },
                  { case: { $eq: ['$source', 'local'] }, then: 3 },
        ...
        { $skip: skip },
        { $limit: limitNum },
        { $project: { sourcePriority: 0 } } // Remove the helper field
      ]),
